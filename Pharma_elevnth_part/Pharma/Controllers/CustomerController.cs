...
namespace Pharma.Controllers
{
    public class CustomerController : Controller
    {
        ...
        if (rec != null)
            {
                if (rec.Email == "admin2@gmail.com" && rec.Password=="admin")
                {
        ...
        [HttpPost]
        public ActionResult Register([Bind(Include = "Id,FirstName,LastName,Email,Password,Title,VisaNumber")] Customer customer)
        {
            if (ModelState.IsValid)
            {
                db.Customers.Add(customer);
        ...
        public ActionResult HomePage(int? page, string sortOrder, string searchString, string currentFilter)
        {
            ViewBag.CurrentSort = sortOrder;
            ViewBag.NameSortParm = String.IsNullOrEmpty(sortOrder) ? "name_desc" : "";
            ViewBag.DateSortParm = sortOrder == "Date" ? "date_desc" : "Date";
            ...
            if (!String.IsNullOrEmpty(searchString))
            {
                products = products.Where(s => s.Category.Name.ToUpper().Contains(searchString.ToUpper()) ||
